...
- **Admin difference:** `protectedHandler` is not enough on its own. After authentication, every handler must pass an authorization check against the admin database: the caller's `cognito_sub` must match an enabled `admin_users` row, and the handler must name the action key it requires (or accept super-admin). A missing key is a 403 `forbidden`. See `docs/admin-access/README.md`. Prefer a single `requireAction("can_...")` helper over ad-hoc checks so the reviewer can grep for it.
