...
- **Admin database:** schema changes go in `prisma-admin/schema.prisma`, then `npx prisma format --config prisma-admin.config.ts`, `npx prisma validate --config prisma-admin.config.ts`, and `npm run prisma:generate`. Creating or applying a migration (`prisma migrate dev --config prisma-admin.config.ts`) touches the database and requires the owner's go-ahead. `docs/admin-access/admin_access.sql` is the source of truth for the `admin_*` tables until they are mirrored into the Prisma schema.
