...
export const ENDPOINT_REGISTRY: readonly EndpointRegistryEntry[] = [
  ...
  {
    key: "admin.me.password.change",
    method: "POST",
    path: "/api/v1/admin/me/password",
    name: "Change own password",
    description:
      "The caller replaces their own Cognito password. Cognito checks the current one, which is the re-authentication the change needs. Acts on no other account.",
  ...
  {
    key: "admin.me.mfa.get",
    method: "GET",
    path: "/api/v1/admin/me/mfa",
    name: "Own MFA status",
    description:
      "Which second factors Cognito has switched on for the caller. Read live from the pool with the caller's access token; nothing is cached.",
    category: "account_security",
    authKind: "admin",
    rateLimit: "api",
    defaults: none,
  ...
  {
    key: "admin.me.mfa.totp.start",
    method: "POST",
    path: "/api/v1/admin/me/mfa/totp",
    name: "Start authenticator enrolment",
    description:
      "AssociateSoftwareToken: mints the shared secret and its otpauth:// URI, shown once and stored nowhere. 503 mfa_not_enabled while the pool's MfaConfiguration is OFF.",
    category: "account_security",
    authKind: "admin",
    rateLimit: "api",
    defaults: none,
  ...
  {
    key: "admin.me.mfa.totp.verify",
    method: "PUT",
    path: "/api/v1/admin/me/mfa/totp",
    name: "Verify authenticator enrolment",
    description:
      "VerifySoftwareToken with a six-digit code, then SetUserMFAPreference to switch the factor on. A code guess, so it carries the reset budget per operator as well as per IP.",
    category: "account_security",
    authKind: "admin",
    rateLimit: "authReset",
    defaults: none,
  ...
  {
    key: "admin.me.mfa.totp.disable",
    method: "DELETE",
    path: "/api/v1/admin/me/mfa/totp",
    name: "Turn off authenticator app",
    description:
      "SetUserMFAPreference with software-token MFA disabled for the caller's own account.",
    category: "account_security",
    authKind: "admin",
    rateLimit: "api",
    defaults: none,
  },
  {
    ...
    name: "List own passkeys",
    description:
      "ListWebAuthnCredentials for the caller. 503 passkeys_not_enabled while the pool has no WebAuthn relying party configured.",
    category: "account_security",
    authKind: "admin",
    rateLimit: "api",
    defaults: none,
  ...
  {
    key: "admin.me.passkeys.start",
    method: "POST",
    path: "/api/v1/admin/me/passkeys",
    name: "Start passkey registration",
    description:
      "StartWebAuthnRegistration: Cognito's CredentialCreationOptions for navigator.credentials.create(). Cognito is the relying party and owns the challenge.",
    category: "account_security",
    authKind: "admin",
    rateLimit: "api",
    defaults: none,
  ...
  {
    key: "admin.me.passkeys.complete",
    method: "PUT",
    path: "/api/v1/admin/me/passkeys",
    name: "Complete passkey registration",
    description:
      "CompleteWebAuthnRegistration with the browser's RegistrationResponseJSON, forwarded to Cognito untouched: it verifies the attestation, the challenge and the origin.",
    category: "account_security",
    authKind: "admin",
    rateLimit: "api",
    defaults: none,
  ...
  {
    key: "admin.me.passkeys.delete",
    method: "DELETE",
    path: "/api/v1/admin/me/passkeys/[id]",
    name: "Delete a passkey",
    description:
      "DeleteWebAuthnCredential. The id is scoped to the caller's own account by Cognito, so a credential belonging to anybody else is simply not found.",
    category: "account_security",
    authKind: "admin",
    rateLimit: "api",
    defaults: none,
  ...
  {
    key: "admin.integrations.currency_pairs.rates",
    method: "GET",
    path: "/api/v1/admin/integrations/currency-pairs/[id]/rates",
    name: "Currency pair history",
    description:
      "One page of what has been downloaded for a watched pair, newest observation day first: the rate, its source (read from the Bank of Canada's own series, or derived from two of them) and when it was fetched.",
    category: "integrations",
    authKind: "admin",
    rateLimit: "api",
    defaults: any("can_read_integrations", "can_write_integrations"),
  ...
  },
  {
    ...
    description:
      "Machine clients (API_KEYS): the newest cached rate per pair, fetching any pair with no rate from today. Optionally dated — `date` for the closest observation on or before a day, `from`/`to` for every observation in a window — answered from the stored history and otherwise by one ranged Bank of Canada call. Unknown pairs join the watch list. Never fails because the provider is down.",
    category: "integrations",
    authKind: "service",
    rateLimit: "service",
    defaults: none,
